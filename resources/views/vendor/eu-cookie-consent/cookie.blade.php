<div style="visibility:{{ ($cookieName == 'facebook') ? 'hidden' : 'visible' }};" >
    ...
    {{-- For the foreced cookies we need this workaround with hidden input because we set the original input disabled @if($cookieName == 'analytics') checked="checked" @endif --}}
    @if(isset($cookie['forced']))
        <input type="hidden" name="{{ $cookieName }}" value="1">
    @endif
    <label for="{{ $cookieName }}" style="font-size: 13px;">
        @if($multiLanguageSupport)
